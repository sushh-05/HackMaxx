    ...
    <section className="space-y-6 pt-6 sm:pt-8 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          {/* Mode filter tabs */}
          <div role="group" aria-label="Filter by event mode" className="flex items-center gap-1 p-1 bg-muted/70 rounded-xl border border-border overflow-x-auto max-w-full">
          ...
          <ul className="divide-y divide-border">
            {rows.map((h, i) => {
              ...
              <li
                ...
                role="button"
                tabIndex={0}
                aria-label={`${h.title} — ${dLeft} days left, prize ${h.prize_inr > 0 ? format(h.prize_inr) : "unlisted"}. Open details.`}
                className="flex items-center gap-3 py-2.5 hover:bg-foreground/[0.02] transition-colors rounded-lg px-1.5 -mx-1.5 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action/60"
                onClick={() =>
                  setDrawerRow({
                    id: h.id,
                    title: h.title,
