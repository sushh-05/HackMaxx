 ...
 * plan view. Keys: ↑↓/j/k move · Enter details · m maxx · / filter · r reload
 * · t sort · Esc back · q quit.
 */
import {
  createCliRenderer,
  BoxRenderable,
...
class App {
  ...
  private async runMaxx(anchor?: Hackathon) {
    ...
    try {
      ...
      log("", C.dim);
      log("  ❯ ▊", C.action);
      this.setStatus("run complete — Esc back to watchlist", C.win);
    } catch {
    ...
    this.hint.content = "Esc back to watchlist · q quit";
  }
