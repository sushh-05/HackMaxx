...
- [ ] Wire real `skillSim` from embeddings into existing Worth Score (`shared/`)
...
- [ ] OpenUI: stream `HackathonCard`, `MaxxingStrategyPanel`, `FiltersBar` via OpenUI Lang. Fallback = plain React (acceptable).
...
- [ ] Submit before deadline; keep commits meaningful throughout

---

## Out of Scope (banned until Phase 5 done)

- Cognito (we use Clerk)
- SNS / SES reminders
- Calendar view
- Teammate finder
- Multi-user picks
